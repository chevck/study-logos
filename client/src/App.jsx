...
import { fetchBreakdown, fetchNotebook, fetchVerse, saveNotebook } from './lib/api.js';
import {
  NOTEBOOK_ID_KEY,
...
export default function App() {
  ...
  const translationCodes = useMemo(
    () => TRANSLATIONS_BY_STUDY_LANGUAGE[studyLanguage] ?? TRANSLATIONS_BY_STUDY_LANGUAGE.eng,
    [studyLanguage],
  );
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          ...
          setToast(
            legacy.length > 0 ? msg.toastNotebookOffline : msg.toastNotebookFail,
          );
        }
      }
    })();
  ...
  const currentSaved = useMemo(() => {
    ...
    const id = notebookEntryId(displayReference, breakdown.word, breakdown.original);
  ...
  const loadVerse = useCallback(
    async (ref, trans, opts = {}) => {
      ...
      try {
        const apiLang =
          opts.bibleApiLanguage ?? studyLanguageToBibleApiLanguage(studyLanguage);
  ...
  const handleStudyLanguageChange = async (code) => {
    const opts = TRANSLATIONS_BY_STUDY_LANGUAGE[code] ?? TRANSLATIONS_BY_STUDY_LANGUAGE.eng;
  ...
  const handleCrossRefClick = async (refString) => {
    ...
    setNavStack((s) => [...s, { reference: displayReference, word: activeWord }]);
    ...
    try {
      ...
      try {
        ...
        await loadBreakdown(wordAtNav, data.reference || fromRef, data.text, translation);
      } catch {
        /* handled */
      }
    }
  ...
  const handleBack = async () => {
    ...
    try {
      ...
      await loadBreakdown(prev.word, data.reference || prev.reference, data.text, translation);
    } catch {
      /* handled */
    }
  };
  ...
  const handleTranslationChange = async (code) => {
    ...
    try {
      ...
      if (activeWord && data.text) {
        await loadBreakdown(activeWord, data.reference || displayReference, data.text, code);
      }
    } catch {
      /* loaders set error */
    }
  ...
  const surfaceCard =
    'rounded-2xl border border-ep-line bg-white shadow-card';

  return (
    <div className="relative min-h-screen overflow-x-hidden font-sans">
      <header className="sticky top-0 z-40 border-b border-ep-line bg-white">
        <div className="mx-auto max-w-5xl px-3 py-3 sm:px-6 sm:py-4">
          <div className="flex items-center justify-between gap-3 md:gap-6">
            <button type="button" onClick={scrollMainTop} className="min-w-0 shrink text-left">
              <Logo brandWord1={t.brandWord1} brandWord2={t.brandWord2} />
            </button>
            <nav className="hidden flex-1 items-center justify-center gap-6 text-sm font-semibold text-gray-600 md:flex lg:justify-start lg:gap-8">
              <button type="button" onClick={scrollMainTop} className="transition hover:text-ep-blue">
                {t.navStudy}
              </button>
              <button type="button" onClick={scrollToSearch} className="transition hover:text-ep-blue">
                {t.navSearch}
              </button>
              <button type="button" onClick={() => setDrawerOpen(true)} className="transition hover:text-ep-blue">
                {t.navNotebook}
              </button>
            </nav>
            <button
          ...
          <nav
            className="mt-3 flex items-center justify-around gap-2 border-t border-ep-line pt-3 text-xs font-semibold text-gray-600 sm:text-sm md:hidden"
            aria-label="Main"
          >
            <button type="button" onClick={scrollMainTop} className="transition hover:text-ep-blue">
              {t.navStudy}
      ...
      <main
        id="study-main"
        ref={mainRef}
        className="mx-auto w-full max-w-5xl px-3 pb-16 pt-6 sm:px-6 sm:pt-8"
      >
        <div className="mb-6 sm:mb-8">
          <h2 className="text-balance text-xl font-extrabold tracking-tight text-ep-ink sm:text-2xl md:text-3xl">
            {displayReference ? (
              <>
                {t.heroWordInsight}{' '}
          ...
          <p className="mt-2 max-w-2xl text-pretty text-sm font-medium leading-snug text-ep-muted sm:text-base sm:leading-normal">
            {t.heroBlurb}
          </p>
        </div>

        <div id="search-panel" className={`mb-6 p-4 sm:mb-8 sm:p-5 md:p-6 ${surfaceCard}`}>
          <div className="mb-5 flex flex-col gap-4 border-b border-ep-line pb-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        {error && (
          <div
            className="mb-6 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-900 shadow-sm sm:mb-8 sm:px-5 sm:py-4"
            role="alert"
          >
            {error}
          </div>
        ...
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3 border-b border-transparent pb-2 sm:mb-6">
        ...
        <div className="w-full">
          <div className={`mb-8 w-full p-4 sm:p-5 md:p-6 ${surfaceCard}`}>
            <div className="mb-5 border-b border-ep-line pb-4">
              <p className="text-xs font-bold uppercase tracking-wide text-gray-500">{t.readingLabel}</p>
              {displayReference ? (
                <p className="mt-1 break-words text-sm font-bold text-ep-ink">
                  {displayReference}{' '}
            ...
            {verseLoading && <p className="text-sm font-semibold text-gray-600">{t.loadingPassage}</p>}
